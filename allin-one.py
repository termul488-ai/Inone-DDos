...
def udp_random_flood(ip, port, duration, packet_size):
    ...
    end_time = time.time() + duration
    packet_count = 0
    print(f"💥 Starting UDP Random flood on {ip}:{port} with {packet_count}-byte...")
    try:
        while time.time() < end_time:
            time.sleep(0.2)
            print(f"💥\033[38;5;21m Starting UDP Random flood on\033[38;5;220m {ip}\033[37m:\033[33m{port}\033[37m with \033[38;5;154m{packet_count}\033[37m-byte.\033[0m")
...
def tcp_syn_flood_single(ip, port, duration):
    ...
    print(f"☑️ Starting TCP SYN flood (Single) on {ip}:{port} for {packet_count} seconds...")
    try:
        while time.time() < end_time:
            time.sleep(0.2)
            print("☑️ Starting TCP SYN flood \033[38;5;220m(single) on \033[32m{ip}:\033[37m{port} \033[33mfor {packet_count} second.\033[0m")
